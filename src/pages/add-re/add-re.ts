...
/*
  Generated class for the AddRe page.

  See http://ionicframework.com/docs/v2/components/#navigation for more info on
  Ionic pages and navigation.
*/
@Component({
  selector: 'page-add-re',
  templateUrl: 'add-re.html'
})
export class AddRePage {
  TramStop;
  tram;
  date;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad AddRePage');
  }
  addRe(){
     this.af.auth.subscribe(auth => {
        if(auth) {
          this.lists = this.af.database.list('/'+auth.uid+'/reminder/');
